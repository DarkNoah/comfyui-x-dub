        ...
        if self.vram_management_enabled:
            ...
            for name, model in self.named_children():
                if name not in model_names:
                    print(f"[VRAM] {name}: not in target model_names, try offload.")
                    if hasattr(model, "vram_management_enabled") and model.vram_management_enabled:
                        print(f"[VRAM] {name}: vram_management_enabled=True, offload is available.")
                        ...
                        print(f"[VRAM] {name}: vram_management_enabled=False, skip offload.")
            getattr(torch, self.device_type).empty_cache()
            # onload models
            for name, model in self.named_children():
                if name in model_names:
                    print(f"[VRAM] {name}: in target model_names, try onload.")
                    if hasattr(model, "vram_management_enabled") and model.vram_management_enabled:
                        print(f"[VRAM] {name}: vram_management_enabled=True, onload is available.")
                        ...
                        print(f"[VRAM] {name}: vram_management_enabled=False, skip onload.")
